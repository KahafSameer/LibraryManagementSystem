...
class DataList{
	...
	var $QueryFieldsTV,
		...
		$ColFieldName, // array of field names for columns
		$ColCaption, // array of column captions
		$ColNumber, // array of column numbers for sorting
		$DataHeight,
		$TableName,
		$TableIcon,
		...
		$QuickSearch, // 0 to 3
		$QuickSearchText, // text for quick search box

		$HighlightColor, // highlight color property
		...
		$TVClasses,
		$DVClasses,
		$SortFields, // array to map sort field numbers to field names
		...
		$QueryFieldsIndexed, // indexed fields for filters
		$FilterPage,     // path to the filter page file
		$ccffv;          // map FilterField values to field captions as stored in ColCaption
	function __construct(){  // PHP 7 compatibility
		$this->DataHeight = 150;
		$this->FilterPage = ''; // initialize FilterPage property
		$this->DataHeight = 150;
		$this->QuickSearchText = ''; // initialize QuickSearchText property
		...
		$this->Template = '';
		$this->HTML = '';
		$this->SortFields = array(); // initialize as empty array by default
		$this->ColCaption = array(); // initialize as empty array by default
		$this->ColNumber = array(); // initialize as empty array by default
	}
	...
	// get post and get variables
		...
		$SelectedID = isset($_REQUEST['SelectedID']) ? $_REQUEST['SelectedID'] : '';
	...
	// begin table and display table title
		...
		// $this->ccffv: map $FilterField values to field captions as stored in ColCaption
			...
			if($rowTemplate=='' || $this->ShowTableHeader){
				// Initialize ColNumber if not already set
				if(empty($this->ColNumber) || count($this->ColNumber) != count($this->ColCaption)){
					$this->ColNumber = array();
					for($ci = 0; $ci < count($this->ColCaption); $ci++){
						$this->ColNumber[$ci] = $ci + 1;
					}
				}
		...
		// end of table view navigation code
			...
			if(($PrintTV || $Print_x) && !empty($_REQUEST['record_selector']) && is_array($_REQUEST['record_selector']) && !in_array($row[$FieldCountTV], $_REQUEST['record_selector'])) continue;
